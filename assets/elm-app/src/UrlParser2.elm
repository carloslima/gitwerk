...
wildcard : Parser (List String -> a) a
wildcard =
    Parser <|
        \{ visited, unvisited, params, value } ->
            case unvisited of
                ...
                next :: rest ->
                    let
                        nextValue =
                            unvisited
                    in
                        [ State (List.append visited rest) [] params (value nextValue) ]
...
(</*>) : Parser a b -> List x -> Parser a b
(</*>) (Parser parseBefore) l =
    Parser (\state -> parseBefore (state))
