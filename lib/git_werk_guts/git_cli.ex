defmodule GitWerkGuts.GitCli do
  @moduledoc """
  ...
  def handle_msg(args, state) do
    Logger.debug "GitWerkGuts.GitCli.handle_msg: unmatched: #{inspect args}"
    {:ok, state}
  end
  ...
  def handle_ssh_msg(args, state) do
    Logger.debug "GitWerkGuts.GitCli.handle_msg.handle_ssh_msg: unmatched: #{inspect args}"
    {:ok, state}
  end
  ...
  def handle_cast(args, state) do
    Logger.debug "GitWerkGuts.GitCli.handle_cast: unmatched: #{inspect args}"
    {:noreply, state}
  end

  @doc false
  def handle_call(args, _, state) do
    Logger.debug "GitWerkGuts.GitCli.handle_call: unmatched: #{inspect args}"
    {:reply, :ok, state}
  end

  @doc false
  def code_change(args, _, state) do
    Logger.debug "GitWerkGuts.GitCli.code_change: unmatched: #{inspect args}"
    {:reply, :ok, state}
  end
end
