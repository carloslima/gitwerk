...
RUN apt update && \
    apt upgrade -y -qq && \
    apt install -y -qq curl git-core libgit2-dev build-essential sudo locales inotify-tools pkgconf
...
RUN cd /tmp && \
    ...
    rm /tmp/*.deb &&\
    apt update && \
    apt install -y -qq elixir=1.5.1-1
...
RUN curl -sSf https://build.travis-ci.org/files/rustup-init.sh | sh -s -- --default-toolchain=1.19.0 -y
